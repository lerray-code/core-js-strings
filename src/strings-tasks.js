...
function removeFirstOccurrences(str, value) {
  const firsHalf = str.slice(0, str.indexOf(value));
  const secondHalf = str.slice(str.indexOf(value) + value.length);
  let result;
  if (str.includes(value)) {
    result = firsHalf + secondHalf;
  } else {
    result = str;
  }
  return result;
...
function removeLastOccurrences(str, value) {
  const firsHalf = str.slice(0, str.lastIndexOf(value));
  const secondHalf = str.slice(str.lastIndexOf(value) + value.length);
  let result;
  if (str.includes(value)) {
    result = firsHalf + secondHalf;
  } else {
...
function countVowels(str) {
  const vouleMatch = str.match(/[aeiouyAEIOUY]/gi);
  return vouleMatch === null ? 0 : vouleMatch.length;
}
...
function isPalindrome(str) {
  const noWhitespece = str.toLowerCase().replace(/[^\w]/g, '');
  const reverseStr = noWhitespece.split('').reverse().join('');
  return noWhitespece === reverseStr;
}
...
function findLongestWord(sentence) {
  ...
  let rez = sentenceInArray[0];
  for (let i = 0; i < sentenceInArray.length; i += 1) {
    if (sentenceInArray[i].length > rez.length) {
      rez = sentenceInArray[i];
    }
  }
  return rez;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strInArray.length; i += 1) {
    strInArray[i] = strInArray[i].split('').reverse().join('');
  }
...
function invertCase(str) {
  const strInArray = str.split('');
  for (let i = 0; i < strInArray.length; i += 1) {
    if (strInArray[i].toUpperCase() === strInArray[i]) {
      strInArray[i] = strInArray[i].toLowerCase();
    } else {
      strInArray[i] = strInArray[i].toUpperCase();
    }
  }
  return strInArray.join('');
}
...
function extractNameFromTemplate(value) {
  const strArray = value.replace('!', '').split(' ');
...
function encodeToRot13(str) {
  ...
  const rotAlfabet = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ,!?';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    result += rotAlfabet[alfabet.indexOf(str[i])];
  }
  return result;
}
...
function getCardId(value) {
  ...
  return cardArray.indexOf(value, 0);
}
